...
### Added
- `MatchEndpoint::scheduledTournamentsByDate()` for SofaScore's current
  `/sport/{sport}/scheduled-tournaments/{date}/page/{page}` calendar endpoint.
- `MatchEndpoint::scheduledEventsByDate()` for flattened event envelopes across
  paginated scheduled tournament buckets.

### Fixed
- Date-based event methods (`gamesByDate()`, `matchesByDate()`, `fightsByDate()`)
  now use SofaScore's current scheduled tournament calendar instead of the
  removed `/sport/{sport}/scheduled-events/{date}` endpoint.
- Symfony bundle config now preserves exact HTTP header names in
  `sofascore_api.http.headers` instead of normalizing dashes to underscores.
...
[2.4.1]: https://github.com/nietonchique/Sofascore-API-Bundle/releases/tag/v2.4.1
